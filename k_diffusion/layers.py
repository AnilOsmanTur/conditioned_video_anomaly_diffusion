...
class SelfAttention1d(ConditionedModule):
    ...
    def forward(self, input, cond):
        n, c, l = input.shape
        qkv = self.qkv_proj(self.norm_in(input, cond))
        qkv = qkv.view([n, self.n_head * 3, c // self.n_head, l]).transpose(2, 3)
        q, k, v = qkv.chunk(3, dim=1)
        scale = k.shape[3] ** -0.25
        att = ((q * scale) @ (k.transpose(2, 3) * scale)).softmax(3)
        att = self.dropout(att)
        y = (att @ v).transpose(2, 3).contiguous().view([n, c, l])
        return input + self.out_proj(y)

# from  IBM / CrossViT repo
...
class CrossAttention1d(ConditionedModule):
    def __init__(self, c_dec, c_enc, n_head, norm_dec, dropout_rate=0.,
        ...
        self.norm_dec = norm_dec(c_dec)
        self.n_head = n_head
        self.q_proj = nn.Conv1d(c_dec, c_dec, 1)
        self.kv_proj = nn.Linear(c_enc, c_dec * 2)
        self.out_proj = nn.Conv1d(c_dec, c_dec, 1)
        self.dropout = nn.Dropout(dropout_rate)

    def forward(self, input, cond):
        n, c, l = input.shape
        q = self.q_proj(self.norm_dec(input, cond))
        q = q.view([n, self.n_head, c // self.n_head, l]).transpose(2, 3)
        kv = self.kv_proj(self.norm_enc(cond[self.cond_key]))
        kv = kv.view([n, -1, self.n_head * 2, c // self.n_head]).transpose(1, 2)
        k, v = kv.chunk(2, dim=1)
        scale = k.shape[3] ** -0.25
        att = ((q * scale) @ (k.T * scale))
        att = att - (cond[self.cond_key_padding][:, None, None, :]) * 10000
        att = att.softmax(3)
        att = self.dropout(att)
        y = (att @ v).transpose(2, 3)
        y = y.contiguous().view([n, c, l])
        return input + self.out_proj(y)
...
class Downsample1d(nn.Module):
    ...
    def forward(self, x):
        ...
        weight = x.new_zeros([x.shape[1], x.shape[1], self.kernel.shape[1]])
        indices = torch.arange(x.shape[1], device=x.device)
        weight[indices, indices] = self.kernel.to(weight)
...
class SinusodialEmbedding(nn.Module):
    def __init__(self, out_dim=32):
        """
        :param out_dim: dimension of the model
        """
        super().__init__()
        assert out_dim % 2 == 0, "Cannot use sin/cos positional encoding with odd dim (got dim={:d})".format(out_dim)
        embedding_max_frequency = torch.tensor(1000.0)
        embedding_min_frequency = torch.tensor(1.0)
    ...
    def forward(self, x):
        x_p = self.angular_speeds * x
        embeddings = torch.cat(
            [torch.sin(x_p), torch.cos(x_p)], dim=-1
        )
        return embeddings
