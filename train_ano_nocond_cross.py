...
def args_parser():
    ...
    p.add_argument('--cond_type', type=str, default='no',
                   help='conditioning data type name')
    p.add_argument('--cond', type=str, default='no',
...
def get_datasets(dataset_name, feat_type, feat_model, no_scale=False):
    from custom_datasets import feature_dataset
    
    dataset_root: str = '../data'

    ds_name2 = 'UCFC' if dataset_name == 'shanghai' else 'shanghai'
    train_set = feature_dataset.ClipDataset(
        root_dir=dataset_root,
        dataset_name=dataset_name,
        feat_model=feat_model,
        cond_model=feat_model,
        feat_type=feat_type,
        cond_type=feat_type,
        split='train',
...
def main():
    ...
    train_set, test_set = get_datasets(dataset_name, feat_type, feat_model, args.ds_no_scale)
    ss_data = train_set.std_data
    
    
    log_dir = f'runs/{dataset_name}/{feat_type}_{feat_model}/nocond_b{args.batch_size}_{search_id}{ext}'
    K.utils.mkdir(log_dir)

    args.name = f'cross_nocond_b{args.batch_size}_{search_id}{ext}'
    ...
    if accelerator.is_main_process:
        try:
            print('trainset:', train_set.dataset_name)
            print('Number of items in dataset:', len(train_set))
            print('Testset:', test_set.dataset_name)
    ...
    if accelerator.is_main_process:
        metrics_log = K.utils.CSVLogger(f'{log_dir}/cross_metrics.csv', ['epoch', 'step', 'auc', 'n_start', 'th_alpha'])
        loss_log = K.utils.CSVLogger(f'{log_dir}/cross_loss.csv', ['epoch', 'step', 'loss'])
        ...
        for epoch in range(args.n_epochs):
            for batch in tqdm(train_dl, disable=not accelerator.is_main_process):
                with accelerator.accumulate(model):
                    ...
                    gloss = model.loss(reals, noise, sigma)  # losses with the batch
